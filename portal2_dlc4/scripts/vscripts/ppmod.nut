...
::ppmod.hook <- function (ent, input, scr, max = -1) {
  ...
  // Otherwise, assign a new hook function
    ...
    ent.GetScriptScope()["Input"+input] <- scr;
    ent.GetScriptScope()["Input"+input.tolower()] <- scr;
  }
...
::ppmod.ray <- class {
  ...
  // Whether to check Potential Visibility Set against players
  usepvs = false;
  ...
  // Updates the ray object's hit entity fraction and handle
  function cast_ent (ent) {

    // Whether to refer to the entity's potential visibility set first
    // This greatly optimizes complex rays cast from the player's eyes
    if (this.usepvs) {
      // Attempt to target special keyword !pvsplayer from this entity
      if (!Entities.FindByName.bindenv({ activator = ent })(null, "!pvsplayer")) return;
    }
  ...
  constructor (start, end, ent = null, world = true, portals = null, ray = null, usepvs = false) {
    ...
    // Store "Use PVS" flag on the ray instance
    this.usepvs = usepvs;
